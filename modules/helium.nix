...
let
  bookmarkList =
    (import ./bookmarks/dev.nix)
    ++ (import ./bookmarks/entertainment.nix)
    ++ (import ./bookmarks/music-production.nix)
  ...
  mkFolderContent =
    ...
    lib.concatMapStrings (
      chunk:
      if chunk.isCards then
        ''
          <div class="cards">
            ${lib.concatMapStrings mkCard chunk.items}
          </div>''
      else
        mkFolder chunk.folder
    ) chunks;
  ...
  mkFolder =
    ...
    let
      iconHtml =
        if folder ? icon then
          ''<span class="material-symbols-outlined folder-icon" aria-hidden="true">${folder.icon}</span>''
        else
          "";
    in
    ''
  ...
  bookmarksFile = pkgs.writeText "helium-bookmarks.json" (
    builtins.toJSON {
      version = 1;
      roots = {
        bookmark_bar = {
                ...
                map (
                  ...
                  if item ? url then
                  ...
                  else
                    {
                      type = "folder";
                      name = item.name;
                      children = formatNative item.bookmarks;
                    }
                ) items;
            in
            formatNative bookmarkList;
        };
        other = {
...
in
{
  ...
  home.activation.installHeliumBookmarks = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
  ...
  home.file.".local/share/helium-startpage/index.html".text = ''
    ...
    <head>
      ...
      <style>
        :root {
          /* 3. Define your custom theme colors here */
          --bg: #1e1e2e;
          --fg: #cdd6f4;
          --accent: #89b4fa;
          --card-bg: #181825;
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        details.folder-section > summary:hover .folder-name { color: var(--accent); }
        details.folder-section > summary:hover .folder-icon { color: var(--accent); }
